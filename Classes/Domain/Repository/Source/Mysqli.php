 ...
 */
class  Mysqli implements SourceInterface {
	...
	*/
	function open($dryRun, $conf, $tx_cabagimport_handler) {
		...
		if(!empty($this->conf['database'])){
			...
			if($this->resource = new mysqli($this->conf['host'],$this->conf['user'],$this->conf['password'],$this->conf['database'])) {
				if($this->resource->select_db($this->conf['database'])) {
					$this->resource->query('SET SESSION group_concat_max_len = 1000000');
					$this->queryResource = $this->resource->query($this->conf['query']);
					if(!$this->queryResource) {
						throw new Exception($LANG->getLL('source.exception.noQueryResult') . ' - ' . $this->resource->error);
					}
				} else {
					throw new Exception($LANG->getLL('source.exception.noDBselected') . ' - ' . $this->resource->connect_error);
				}
			} else {
				throw new Exception($LANG->getLL('source.exception.noConnectionPossible') . ' - ' . $this->resource->connect_error);
			}
		} else {
			// use regular typo3 db
			$GLOBALS['TYPO3_DB']->sql_query('SET SESSION group_concat_max_len = 1000000');
			$this->queryResource = $GLOBALS['TYPO3_DB']->sql_query($this->conf['query']);
			if(!$this->queryResource) {
				throw new Exception($LANG->getLL('source.exception.noQueryResult').$this->queryResource->error);
			}
		}

		// debug
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('connected to mysqli host/db', 'cabag_import', -1, array('conf' => $this->conf));
	}
	...
	*/
	function getNextRow() {
		...
		if($resultArray = $this->queryResource->fetch_assoc()) {
			return $resultArray;
		} else {
			return false;
		}
