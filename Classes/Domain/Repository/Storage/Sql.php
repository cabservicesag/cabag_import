 ...
 */
class Sql implements StorageInterface {
	// defined keyFields
	protected $keyFields;
	...
	*/
	...
	function getKeyFieldsQuery($row){
		...
		return	$keyFieldsQuery = $this->keyFields . '=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($row[$this->keyFields], 'pages') . ' ';
	}
	...
	*/
	...
	public function __destruct() {
		if(!empty($this->insertCache) && !empty($this->insertCacheTable)) {
			if(!$this->resource) {
				// Insert the new row to the DB
				$dbRes = $GLOBALS['TYPO3_DB']->exec_INSERTmultipleRows($this->insertCacheTable, $this->insertCache);
