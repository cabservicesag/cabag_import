<?php
namespace Cabag\CabagImport\Handler;
/***************************************************************
*  Copyright notice
*
...
class ImportHandler {
	...
	public $currentRowConf;

	// Pid for the current row
	public $currentPid;

	// field proc object
	public $currentFieldProc;
	...
	// Objects
	public $source = false;
	public $interpret = false;
	public $db = false;
	public $loghandler = false;

	// global counter for records
	public $countOfRecords = 0;

	// arrays with available "modules"
	// defined in ext_localconf.php as SC_OPTIONS
	public $availableFieldProcs = array();
	public $availableSources = array();
	public $availableInterprets = array();
	public $availableLoghandlers = array();
	...
	*/
	public function __construct($conf) {
		...
		if(!($this->db instanceOf \Cabag\CabagImport\Domain\Repository\Storage\StorageInterface)){
		...
		if(!empty($this->conf['source.']['interpret'])){
			if(!empty($this->availableInterprets[$this->conf['source.']['interpret']])){
				...
				if(!($this->interpret instanceOf \Cabag\CabagImport\Interpret\InterpretInterface)){
		...
		if(!empty($this->conf['source'])){
			if(!empty($this->availableSources[$this->conf['source']])){
				...
				if(!($this->source instanceOf \Cabag\CabagImport\Domain\Repository\Source\SourceInterface)){
					$this->source = false;
				}
			}
		}
	...
	*/
	function main($dryRun=true) {
		...
		$this->source->open($this->dryMode, $this->conf['source.'], $this);
		...
		if(!empty($this->conf['mapping.'])) {
			// check if we truncate the table before import
			if(!empty($this->conf['storage.']['truncateBeforeImport'])&& $this->conf['storage']=='sql') {
				$this->db->truncate($this->conf['handler.']['table']);
			}
			// go trought the rows
			while($this->currentRowRaw = $this->source->getNextRow()) {
				...
				try {
					...
					if(empty($this->currentPid)){
						if(empty($this->conf['handler.']['defaultPid']) && $this->conf['handler.']['defaultPid'] != 0){
	...
	static function getConf($importConfIdent) {
		...
		if (empty($importConfIdent)) {
			...
			if (is_numeric($importConfIdent) && !$extConf['onlyConfigsFromFile']) {
				$confRes = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cabagimport_config', 'tx_cabagimport_config.uid = ' . intval($importConfIdent) . ' ' . \TYPO3\CMS\Backend\Utility\BackendUtility::deleteClause('tx_cabagimport_config'), '', 'title ASC');
				...
				switch($positionvalue) {
					default:
						...
						if(!empty($this->availableFieldProcs[$positionvalue])){
							...
							if($this->currentFieldProc instanceof \Cabag\CabagImport\Fieldproc\FieldprocInterface) {
