 ...
 */
class  SaveToRawRow implements FieldprocInterface {
	...
	* main()
	* 
	* - saves the current value back to the row
	* 
	* @param	array	mapping configuration for the current part includes required option and stack
	* @param	tx_cabagimport_handler The executing instance
	...
	*/
	function main ($stackPartConf=false, $object_handler) {
		...
		if (!empty($stackPartConf['field']) && is_string($stackPartConf['field'])) {
			$object_handler->currentRowRaw[$stackPartConf['field']] = $object_handler->currentFieldValue;
		}
