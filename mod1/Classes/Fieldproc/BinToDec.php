 ...
 */
class BinToDec implements FieldprocInterface {
	
	/**
	* main()
	* 
	* Transform the current value from binary to decimal
	* 
	* @param	array	mapping configuration for the current part includes required option and stack
	* @param	\Cabag\CabagImport\Handler\ImportHandler	$object_handler	field with the data
	...
	*/
	function main($stackPartConf=false, $object_handler) {		
		// bindec the current value
		$object_handler->currentFieldValue = bindec($object_handler->currentFieldValue);
