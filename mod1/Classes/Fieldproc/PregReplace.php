 ...
 */
class  PregReplace implements FieldprocInterface {
	...
	*/
	function main($stackPartConf=false, $object_handler) {
		...
		if(substr($stackPartConf['from'],0,1) != '/') {
			$stackPartConf['from'] = '/'.$stackPartConf['from'].'/';
		}
		
		$object_handler->currentFieldValue = preg_replace($stackPartConf['from'], $stackPartConf['to'], $object_handler->currentFieldValue);
		return true;
	}
}